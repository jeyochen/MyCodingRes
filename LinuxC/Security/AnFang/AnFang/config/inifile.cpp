...
/*=========================================================================
   FilterSpace
  -------------------------------------------------------------------------
   Job : Delete string's before or after workspace
==========================================================================*/
void   CIniFile::FilterSpace( char *str,int size)
{
	if ( (NULL == str) || (0 >= size) )
	{
		return;
...
bool CIniFile::FindKey  (CCHR *pSection, CCHR *pKey, EFIND *pList)
{
	...
	while (pEntry != NULL)
	{
		...
		if (pEntry->Type == tpKEYVALUE)
		{
			...
			{
				*pText = 0;
				// filter before or after workspace of key
				FilterSpace(Text,strlen(Text));
				...
				*pText = '=';
				...
				{
				   char *p = pText+1;
				   // filter before or after workspace of value
				   FilterSpace(p,strlen(p));
