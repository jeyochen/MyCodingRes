...
CTemperHumiCtrl::CTemperHumiCtrl(void)
    :m_fd(-1)
{
}
...
//打开串口
bool CTemperHumiCtrl::OpenSerialPort(uint8 port)
{
...
//设置串口波特率
bool CTemperHumiCtrl::SetBaudrate(int buad)
{
    struct termios opt;
...
//设置奇偶校验
bool CTemperHumiCtrl::SetParity(uint8 databits, uint8 stopbits, uint8 parity)
{
    struct termios opt;
...
//读数据
int CTemperHumiCtrl::ReadData(unsigned char *pbuf, const int len, const uint8 timeout)
{
    int left_len = len;
    int nfds = 0;
    fd_set read_fds;
...
//关闭串口
bool CTemperHumiCtrl::ClosePort()
{
    close(m_fd);
    m_fd = -1;
...
//串口是否打开
bool CTemperHumiCtrl::IsOpen()
{
    return -1 != m_fd;
}

// 计算CRC校验码
void CTemperHumiCtrl::CRC16_Modbus(const uint8 *ptr, uint8 len, uint8 crc[2])
{
...
// 字符串转16进制
void CTemperHumiCtrl::StrToHex(uint8 *pbDest, const char *pbSrc, int nLen)
{
    char h1,h2;
    uint8 s1,s2;
...
// 十六机制转字符串
void CTemperHumiCtrl::HexToStr(uint8 *pbDest, uint8 *pbSrc, int nLen)
{
    char ddl,ddh;
...
// 删除空格
int CTemperHumiCtrl::TrimSpace(char* pDest, const char *src, int nLen)
{
    int j = 0;
    for (int i = 0; i < nLen; i++)
    {
