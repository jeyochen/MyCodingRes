...
bool CSerialPort::OpenSerialPort(uint8 port)
{
...
bool CSerialPort::SetBaudrate(uint16 buad)
{
    struct termios opt;
    ...
    for (i = 0; i < arr_len; i++)
    {
        if (buad == name_arr[i])
        {
            ...
            {
                perror("SetBaudrate tcsetattr fail!\n");
                return false;
            }
            break;
        }
    ...
    if (i >= arr_len)
    {
        printf("SetBaudrate failed,error value:%d\n", buad);
        return false;
    }
...
bool CSerialPort::SetParity(uint8 databits, uint8 stopbits, uint8 parity)
{
    struct termios opt;
...
bool CSerialPort::WriteData(const char *pbuf, const int len, const uint8 timeout)
{
	...
	{
		printf("send length must lower 1024\n");
		return -1;
	}
	...
	if (temp_len % 2 != 0)
	{
		printf("HEX length error, len % 2 == 0\n");
		return false;;
	}
...
int CSerialPort::ReadData(unsigned char *pbuf, const int len, const uint8 timeout)
{
    int left_len = len;
    int nfds = 0;
    fd_set read_fds;
...
void CSerialPort::CRC16_Modbus(const uint8 *ptr, uint8 len, uint8 crc[2])
{
...
void CSerialPort::StrToHex(uint8 *pbDest, const char *pbSrc, int nLen)
{
    char h1,h2;
    uint8 s1,s2;
...
void CSerialPort::HexToStr(uint8 *pbDest, uint8 *pbSrc, int nLen)
{
	char ddl,ddh;
...
int CSerialPort::TrimSpace(char* pDest, const char *src, int nLen)
{
	int j = 0;
	for (int i = 0; i < nLen; i++)
	{
